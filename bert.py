...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        ### TODO
        d_sqrt = key.shape[-1]**.5
        score = (query@key.permute(0,1,3,2)) / d_sqrt
        masked_score = score + attention_mask
        softmax = torch.softmax(masked_score,axis=-1) 
        attention = self.dropout(softmax)@value 
        return attention.permute(0,2,1,3).flatten(-2)
...
class BertLayer(nn.Module):
    ...
    def add_norm(self, input, output, dense_layer, dropout, ln_layer):
        ...
        """
        ### TODO
        dense_output = dropout(dense_layer(output))
        res_sum = input + dense_output
        res_sum = ln_layer(res_sum)
        return res_sum
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        normed_attention = self.add_norm(hidden_states,attention,self.attention_dense,self.attention_dropout,self.attention_layer_norm)
        feed_forward_attention = self.interm_af(self.interm_dense(normed_attention))
        normed_attention = self.add_norm(normed_attention,feed_forward_attention,self.out_dense,self.out_dropout,self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.position_ids = position_ids #added
        # bert encoder
    ...
    def embed(self, input_ids):
        ...
        ### TODO
        added_embs = inputs_embeds + pos_embeds + tk_type_embeds
        hidden_states = self.embed_dropout(self.embed_layer_norm(added_embs))
